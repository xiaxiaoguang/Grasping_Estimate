...
def main():
    ...
    train_dataset = PoseDataset(config, mode="train", scale=10)
    val_dataset = PoseDataset(config, mode="val", scale=10)

    # preprocess_pose_data(config, mode="train")
    # preprocess_pose_data(config, mode="val")
    # exit()
    # train_dataset = PreprocessedPoseDataset("processed/power_drill/train",config)
    # val_dataset = PreprocessedPoseDataset("processed/power_drill/val",config)
    ...
    for it in trange(cur_iter, config.max_iter):
        ...
        if it % config.val_interval == 0:
            with torch.no_grad():
                ...
                logger.log(
                    {
                        k: np.array([dic[k] for dic in result_dicts]).mean()
