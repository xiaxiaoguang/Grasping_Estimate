...
def set_seed(seed: int):
    # 1. Set environment variable for Python's hash seed (important for dataloader shuffling)
    os.environ["PYTHONHASHSEED"] = str(seed)

    # 2. Set seeds for all relevant libraries
    random.seed(seed)
    np.random.seed(seed)
...
def transform_grasp_pose(
    ...
    est_trans: np.ndarray
        Estimated translation vector (object to camera frame).
    est_rot: np.ndarray
        Estimated rotation matrix (object to camera frame).
    cam_trans: np.ndarray
        Camera's translation vector (camera to robot frame).
    cam_rot: np.ndarray
        Camera's rotation matrix (camera to robot frame).
    ...
    """
    # Step 1: Grasp pose in object frame
    pos_obj = grasp.trans        # (3,)
    rot_obj = grasp.rot          # (3, 3)

    # Step 2: Transform to camera frame
    pos_cam = est_rot @ pos_obj + est_trans       # (3,)
    rot_cam = est_rot @ rot_obj                   # (3, 3)

    # Step 3: Transform to robot frame
    pos_robot = cam_rot @ pos_cam + cam_trans     # (3,)
    rot_robot = cam_rot @ rot_cam                 # (3, 3)

    # Return transformed Grasp object
    return Grasp(trans=pos_robot, rot=rot_robot, width=grasp.width)
